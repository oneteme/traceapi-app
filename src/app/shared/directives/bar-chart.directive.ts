...
import { ChartConfig, ChartView, DataSet, mergeDeep, toCategoriesFn } from "./chart.model";
...
export class BarChartDirective implements ChartView, OnDestroy {
  ...
  @Input() set data(objects: any[]) {
    ...
    if (objects?.length) {
      ...
      series = dataSet.data(mappers, 0).map(d => ({ name: d.mapper.group ? d.name + '_' + d.mapper.group : d.name, color: d.mapper.color, group: d.mapper.group, data: d.data }));

      // series = mappers.map(m => ({ name: m.label, color: m.color, data: objects.map(o => {
