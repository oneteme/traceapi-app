...
// Issue : Palette de couleur / tableau de couleur / degradé (min/max)
...
export interface DataMapper { // add unit,
...
// export interface SimpleDataMapper extends DataMapper {
//   color?: string;
// }

// export interface StackedDataMapper extends DataMapper {
//   stack?: string;
//   group?: string;
//   color?: string | string[];
// }
...
export class DataSet {
  ...
  stackedData(mapper: DataMapper, defaultValue: any = null): RowSet[] {
    ...
    return Object.entries(map)
      .map((arr: [string, any]) => {
        return { name: arr[0], data: this.labels.map(l => isNaN(arr[1][l]) ? defaultValue : arr[1][l]), mapper: mapper }
      })
  };
}
